...
The base application logic is adapted from prashantgohel321/DevOps-Project-Two-Tier-Flask-App. My work focused on provisioning the cloud infrastructure, resolving build and runtime constraints, and building the automated deployment pipeline.

🛠️ Tech Stack & Ecosystem
LayerTechnologies UsedCloud & InfrastructureAWS EC2 (Ubuntu), Security GroupsContainerization & OrchestrationDocker, Docker ComposeCI/CD AutomationJenkins (Pipeline-as-Code), GitHubBackend ApplicationPython 3, Flask, PyMySQL, CryptographyDatabase TierMySQL 8
...
Push: Code is committed and pushed to GitHub.
Trigger: Jenkins detects the new commit (via Poll SCM or a GitHub webhook).
Pipeline Execution: Jenkins clones the repo, runs the declarative Jenkinsfile, and invokes Docker Compose.
Orchestrated Rollout: Docker Compose tears down old containers, rebuilds the updated Flask image, and brings the network services back up.
...
📂 Project Structure
bash├── diagrams/              # Architecture and workflow diagrams
...
bash   git clone https://github.com/Amirtha655/two-tier-flask-cicd.git
   cd two-tier-flask-cicd

Spin up the environment with Docker Compose:

bash   docker compose up -d --build

Access the web application at http://<your-ec2-public-ip>:5000

Automated CI/CD Setup

Install Docker and Jenkins on your EC2 instance.
Add the jenkins user to the docker group:

bash   sudo usermod -aG docker jenkins && sudo systemctl restart jenkins

Create a Pipeline project in Jenkins pointing to this GitHub repository (Pipeline script from SCM → Jenkinsfile, branch main).
Enable automatic deploys with one of:
...
Push a commit and watch Jenkins build and deploy automatically.
ArtifactsTwo tier flask cicd guideDocument · MD ContentHere is the complete, raw markdown code blocks for easy copying.

```markdown
# 🚀 Two-Tier Flask App — End-to-End CI/CD Deployment on AWS

A production-ready, two-tier web application architecture featuring a Python Flask backend and a MySQL database. This project demonstrates full infrastructpasted
